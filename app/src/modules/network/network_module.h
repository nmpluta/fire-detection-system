...
/**
 * @brief Network module states
 */
enum network_module_state {
	NETWORK_MODULE_STATE_RUNNING,
	NETWORK_MODULE_STATE_DISCONNECTED,
	NETWORK_MODULE_STATE_DISCONNECTED_IDLE,
	NETWORK_MODULE_STATE_DISCONNECTED_SEARCHING,
	NETWORK_MODULE_STATE_CONNECTED,
	NETWORK_MODULE_STATE_DISCONNECTING
};

/**
 * @brief Network module message types
 */
enum network_msg_type {
	 ...
	 * and it will not resume before explicitly requested to do so by a NETWORK_CONNECT_REQUEST
	 * message.
	 */
	NETWORK_SEARCH_STOP_REQUEST,
	 ...
	 */
	NETWORK_SYSTEM_MODE_SET_LTEM_REQUEST,

	/* Request to set the system mode to only use NB-IoT access technology.
	 * Note that the network module must be disconnected and idle, ie. not actively searching
	 * for the system mode to be set.
	 */
	NETWORK_SYSTEM_MODE_SET_NBIOT_REQUEST,
	 ...
	 * for the system mode to be set.
	 */
	NETWORK_SYSTEM_MODE_SET_LTEM_NBIOT_REQUEST,
...
/**
 * @brief Network module message structure
 */
struct network_msg {
	enum network_msg_type type;
	union {
		/** Contains the currently configured system mode.
		 *  system_mode is set for NETWORK_MODULE_SYSTEM_MODE_RESPONSE events
		 */
		enum lte_lc_system_mode system_mode;

		/** Contains the current PSM configuration.
		 *  psm_cfg is valid for NETWORK_MODULE_PSM_PARAMS events.
		 */
...
/* Helper macro for casting ZBUS message to network_msg */
