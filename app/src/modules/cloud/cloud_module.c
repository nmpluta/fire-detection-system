...
/* Register log module */
LOG_MODULE_REGISTER(cloud_module, CONFIG_CLOUD_MODULE_LOG_LEVEL);
...
BUILD_ASSERT(CONFIG_CLOUD_MODULE_WATCHDOG_TIMEOUT_SECONDS >
 ...
 * sensor_chan, POWER_CHAN, and LOCATION_CHAN are optional and are only included if the
 * corresponding module is enabled.
 */
#define CHANNEL_LIST(X)                                                                            \
	X(network_chan, struct network_msg)                                                        \
	X(cloud_chan, struct cloud_msg)                                                            \
	X(sensor_chan, struct sensor_msg)
...
ZBUS_CHAN_DEFINE(cloud_chan, struct cloud_msg, NULL, NULL, ZBUS_OBSERVERS_EMPTY,
...
static const struct smf_state states[] = {
	...
	[STATE_DISCONNECTED] = SMF_CREATE_STATE(state_disconnected_entry, state_disconnected_run,
						NULL, &states[STATE_RUNNING], NULL),

	[STATE_CONNECTING] =
		SMF_CREATE_STATE(state_connecting_entry, NULL, NULL, &states[STATE_RUNNING],
				 &states[STATE_CONNECTING_ATTEMPT]),

	[STATE_CONNECTING_ATTEMPT] = SMF_CREATE_STATE(state_connecting_attempt_entry, NULL, NULL,
						      &states[STATE_CONNECTING], NULL),

	[STATE_CONNECTING_BACKOFF] =
		SMF_CREATE_STATE(state_connecting_backoff_entry, state_connecting_backoff_run,
				 state_connecting_backoff_exit, &states[STATE_CONNECTING], NULL),

	[STATE_CONNECTED] =
		SMF_CREATE_STATE(state_connected_entry, NULL, state_connected_exit,
				 &states[STATE_RUNNING], &states[STATE_CONNECTED_READY]),
...
{
	uint32_t backoff_time = CONFIG_CLOUD_MODULE_BACKOFF_INITIAL_SECONDS;

	/* Calculate backoff time */
	if (IS_ENABLED(CONFIG_CLOUD_MODULE_BACKOFF_TYPE_EXPONENTIAL)) {
		backoff_time = CONFIG_CLOUD_MODULE_BACKOFF_INITIAL_SECONDS << (attempts - 1);
	} else if (IS_ENABLED(CONFIG_CLOUD_MODULE_BACKOFF_TYPE_LINEAR)) {
		backoff_time =
			CONFIG_CLOUD_MODULE_BACKOFF_INITIAL_SECONDS +
			((attempts - 1) * CONFIG_CLOUD_MODULE_BACKOFF_LINEAR_INCREMENT_SECONDS);
	}

	/* Limit backoff time */
	if (backoff_time > CONFIG_CLOUD_MODULE_BACKOFF_MAX_SECONDS) {
		backoff_time = CONFIG_CLOUD_MODULE_BACKOFF_MAX_SECONDS;
	}
...
static void check_credentials(void)
{
	...
	if (status == 0) {
		LOG_INF("Credentials required for nRF Cloud exist.");
		return;
	}
	...
	if (!IS_ENABLED(CONFIG_NRF_PROVISIONING)) {
		 ...
		 */
		LOG_ERR("Credentials are missing or incomplete.");
		// LOG_WRN("Disabling all interfaces to save power.");
		// TODO: Add sending message to network module to disable all interfaces as it
		// doesn't make sense to keep them up.
	}
	k_sleep(K_FOREVER);
}
...
static void shadow_get(bool delta_only)
{
	...
	err = nrf_cloud_coap_shadow_get(msg.response.buffer, &msg.response.buffer_data_len,
					delta_only, COAP_CONTENT_FORMAT_APP_CBOR);
	if (err) {
	...
	err = nrf_cloud_coap_patch("state/reported", NULL, msg.response.buffer,
				   msg.response.buffer_data_len, COAP_CONTENT_FORMAT_APP_CBOR, true,
				   NULL, NULL);
	if (err) {
...
static void state_connected_ready_run(void *obj)
{
	int err;
	struct cloud_state_object const *state_object = obj;
	bool confirmable = IS_ENABLED(CONFIG_CLOUD_MODULE_CONFIRMABLE_MESSAGES);
	...
	if (state_object->chan == &network_chan) {
		...
		case NETWORK_QUALITY_SAMPLE_RESPONSE:
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_CONEVAL,
							 ...
							 NRF_CLOUD_NO_TIMESTAMP, confirmable);
			if (err) {
...
#if defined(CONFIG_SENSOR_MODULE)
	if (state_object->chan == &sensor_chan) {
		struct sensor_msg msg = MSG_TO_SENSOR_MSG(state_object->msg_buf);
		...
		if (msg.type == SENSOR_SAMPLE_RESPONSE) {
			...
			int64_t timestamp = NRF_CLOUD_NO_TIMESTAMP;
#endif /* CONFIG_SENSOR_MODULE_TIMESTAMP */

			err = nrf_cloud_coap_sensor_send(NRF_CLOUD_JSON_APPID_VAL_TEMP,
							 sensor_value_to_double(&msg.temperature),
							 timestamp, confirmable);
			if (err) {
				LOG_ERR("nrf_cloud_coap_sensor_send, error: %d", err);
				send_request_failed();
				return;
			...
			err = nrf_cloud_coap_sensor_send(NRF_CLOUD_JSON_APPID_VAL_AIR_PRESS,
							 sensor_value_to_double(&msg.pressure) *
								 1000, /* Convert from kPa to Pa */
							 timestamp, confirmable);
			if (err) {
				LOG_ERR("nrf_cloud_coap_sensor_send, error: %d", err);
				send_request_failed();
			...
			err = nrf_cloud_coap_sensor_send(NRF_CLOUD_JSON_APPID_VAL_HUMID,
							 sensor_value_to_double(&msg.humidity),
							 timestamp, confirmable);
			if (err) {
				LOG_ERR("nrf_cloud_coap_sensor_send, error: %d", err);
				send_request_failed();
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_CO2,
							 sensor_value_to_double(&msg.co2),
							 timestamp, confirmable);
			if (err) {
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_VOC,
							 sensor_value_to_double(&msg.voc),
							 timestamp, confirmable);
			if (err) {
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_PM1_0,
							 sensor_value_to_double(&msg.pm1_0),
							 timestamp, confirmable);
			if (err) {
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_PM2_5,
							 sensor_value_to_double(&msg.pm2_5),
							 timestamp, confirmable);
			if (err) {
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_PM10,
							 sensor_value_to_double(&msg.pm10),
							 timestamp, confirmable);
			if (err) {
			...
			err = nrf_cloud_coap_sensor_send(CUSTOM_JSON_APPID_VAL_CO,
							 sensor_value_to_double(&msg.co), timestamp,
							 confirmable);
			if (err) {
...
static void cloud_module_thread(void)
{
	...
	const uint32_t wdt_timeout_ms =
		(CONFIG_CLOUD_MODULE_WATCHDOG_TIMEOUT_SECONDS * MSEC_PER_SEC);
	const uint32_t execution_time_ms =
		(CONFIG_CLOUD_MODULE_MSG_PROCESSING_TIMEOUT_SECONDS * MSEC_PER_SEC);
	...
	k_sleep(K_SECONDS(10)); /* Wait for other modules to initialize */
...
K_THREAD_DEFINE(cloud_module_thread_id, CONFIG_CLOUD_MODULE_THREAD_STACK_SIZE, cloud_module_thread,
