...
LOG_MODULE_DECLARE(cloud_module, CONFIG_CLOUD_MODULE_LOG_LEVEL);
...
static int cmd_publish(const struct shell *sh, size_t argc, char **argv)
{
	...
	err = snprintk(msg.payload.buffer, sizeof(msg.payload.buffer), PAYLOAD_MSG_TEMPLATE,
	...
	(void)shell_print(sh, "Sending on payload channel: %s (%d bytes)", msg.payload.buffer,
