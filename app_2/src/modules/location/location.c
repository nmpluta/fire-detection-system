...
ZBUS_CHAN_DEFINE(LOCATION_CHAN, struct location_msg, NULL, NULL, ZBUS_OBSERVERS_EMPTY,
		 ZBUS_MSG_INIT(0));
...
{
	int err;
	struct location_msg location_msg = {.type = status};
...
{
	int err;
	struct location_msg location_msg = {.type = LOCATION_AGNSS_REQUEST,
...
{
	int err;
	struct location_msg location_msg = {.type = LOCATION_GNSS_DATA,
...
static void location_module_thread(void)
{
	...
	while (true) {
		...
		err = zbus_sub_wait_msg(&location, &location_state.chan, location_state.msg_buf,
					zbus_wait_ms);
		if (err == -ENOMSG) {
			continue;
		} else if (err) {
...
K_THREAD_DEFINE(location_module_thread_id, CONFIG_APP_LOCATION_THREAD_STACK_SIZE,
		location_module_thread, NULL, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0, 0);
