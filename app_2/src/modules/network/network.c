...
static void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_PSM_UPDATE: {
		...
		LOG_DBG("PSM parameters received, TAU: %d, Active time: %d", msg.psm_cfg.tau,
...
static void network_module_thread(void)
{
	...
	while (true) {
		...
		err = zbus_sub_wait_msg(&network, &network_state.chan, network_state.msg_buf,
					zbus_wait_ms);
		if (err == -ENOMSG) {
			continue;
		} else if (err) {
...
K_THREAD_DEFINE(network_module_thread_id, CONFIG_APP_NETWORK_THREAD_STACK_SIZE,
